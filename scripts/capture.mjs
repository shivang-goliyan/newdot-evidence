...
const [width, height] = (process.env.VIEWPORT ?? '1512x982').split('x').map(Number);
...
const shoot = (page, name) => page.screenshot({path: path.join(OUT_DIR, `${name}.png`), fullPage: false});
